...
class DatabaseRemoveShortURLUseCaseTests: XCTestCase {

    let mockClient = MockShortURLDatabaseRemoveClient()
    ...
    func testDatabaseSaveShortURLUseCase_executeWithValidData_ShouldReturnSuccess() {
        ...
        let sut = DatabaseRemoveShortURLUseCase(databaseClient: mockClient)

        // Act
        sut.execute(validShortURLModel) { [weak self] success in
            // Assert
            XCTAssertTrue(success)
            XCTAssertEqual(self?.mockClient.removedModel, self?.validShortURLModel)
        }
    }
